...
while running:
    ...
    if not paused:
        ...
        if aircraft_type == "JET":
            ...
            actual_turn = turn_speed * (plane_speed / (1.0 + plane_speed**2 * 0.010519))
        ...
        # Throttle Detent at 80% (Afterburner Gate)
        if plane_throttle == 80.0:
            if keys[pygame.K_w]:
                detent_timer += 1
                if detent_timer >= 60: # 1 second at 60 FPS
                    plane_throttle += 1.0
                    detent_timer = 0
            ...
            if keys[pygame.K_w]:
                new_throttle = plane_throttle + 1.0
                if plane_throttle < 80.0 and new_throttle >= 80.0:
                    plane_throttle = 80.0
                    detent_timer = 0
                else:
                    plane_throttle = min(100.0, new_throttle)
            if keys[pygame.K_s]:
                new_throttle = plane_throttle - 1.0
                if plane_throttle > 80.0 and new_throttle <= 80.0:
                    plane_throttle = 80.0
    ...
    speed_txt = font.render(str(int(current_spd * 100)), True, GREEN)
...
# =====================================================
    ...
    pygame.draw.line(
        screen,
        BLACK,
        (gauge_x + gauge_w // 2, gauge_y),
        (gauge_x + gauge_w // 2, gauge_y + gauge_h),
        8
    )
